...
//            config.getNowDate(new String());
    ...
    public static void main(int myport) throws Exception {   //将端口作为参数传入main函数，用不同端口模拟不同RSU
      ...
      //  int ff = 0;
        String content = null;
        ArrayList<String> tokenbuffer = new ArrayList<String>();
            ...
            try {
                ...
                switch (type) {
                    case 0:     //RSU间通信，收到前一区域RSU给来的token，解密并计算其哈希值，将解密后的token与其哈希值存储。
                        ...
                        config.getNowDate("Got a token from another RSU.");
                        ...
                        tokenbuffer.add(tokengot);
                        hashbuffer.add(hashgot);
                        config.getNowDate("The token and its hash value have been stored.");
                        break;
                    ...
                    case 1:     // 收到车辆离开请求：查询信誉值然后制作token字段，将其加密后传给白板车以及下一区域RSU
                        ...
                        DatagramPacket sendPacktoCar = new DatagramPacket(msg2car, msg2car.length, inetAddrForBroad, 10999);
                        server.send(sendPacktoCar);   //将token发送给车
                        DatagramPacket sendPacktoRSU2 = new DatagramPacket(msg2RSU2, msg2RSU2.length, inetAddrForBroad, 9999);
                    ...
                    case 3:     //收到小车发来的Token的哈希值，与缓存中的哈希值进行对比，若成功，则使用token中的信息为小车注册身份，并将身份文件返回给小车，在缓存中将该token以及对应的哈希值删除。
                        ...
                        if(hashbuffer.contains(tokenhash)){  //对比哈希值
                            ...
                            String tokenString = tokenbuffer.get(ind);                //根据索引得到相应的Token
                            JSONObject token1 = JSONObject.parseObject(tokenString);  //将字符串形式的token转换为json
                            ...
                            sendPacktoCar = new DatagramPacket(id2car, id2car.length, inetAddrForBroad, 10999);
                            server.send(sendPacktoCar);    //将存有身份信息的消息传给白板车
                            config.getNowDate("The vehicle has been register and the ID information has been transmitted to the vehicle.");
                            hashbuffer.remove(ind);        //删除缓存
                            tokenbuffer.remove(ind);
